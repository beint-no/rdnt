...
def resolve_output_path(folder: Path, fm: dict) -> Path:
    fi = (fm or {}).get("featured_image", "")
    if fi:
        p = Path(fi.lstrip("/"))
        if p.parts and p.parts[0] == "blogs":  # running from blogs/
            p = Path(*p.parts[1:])
        if p.suffix.lower() != ".svg":
            p = folder / (folder.name + "-image.svg")
        else:
...
def main():
    ap = argparse.ArgumentParser(description="Generate centered SVG article images from frontmatter.")
    ap.add_argument("--root", default="nyheter", help="Folder under blogs/ (default: teori)")
    ap.add_argument("--width", type=int, default=1200)
    ap.add_argument("--height", type=int, default=630)
    ap.add_argument("--bg", default="#f3f4f6", help="Background color (default: slightly grey)")
    ap.add_argument("--fg", default="#0b1220", help="Text color")
    args = ap.parse_args()

    base = Path.cwd() / args.root
    if not base.exists():
    ...
    for folder in sorted([d for d in base.iterdir() if d.is_dir()]):
        ...
        out_path = resolve_output_path(folder, fm)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        svg = make_svg(title, desc, width=args.width, height=args.height, bg=args.bg, fg=args.fg)
